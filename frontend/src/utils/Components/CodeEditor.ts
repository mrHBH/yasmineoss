...
class CodeEditor {
  ...
  async laodfiletree() {
    const filetree = `
        ...
        `;
    let sidebarz = this.component.HtmlElement.querySelector(
      "#sidebar-container"
    ) as HTMLElement;
    if (sidebarz) {
  ...
  updateStatusBar = (filename) => {
    let statusBar = this.component.HtmlElement.querySelector(
      "#editor-status-bar"
    ) as HTMLElement;
    if (!statusBar) {
  ...
  async loadworkspace(name) {
    ...
    websocket.onmessage = async (event) => {
      const result = JSON.parse(event.data);
      console.log(result);
      let container = this.component.HtmlElement.querySelector(
        "#monaco-editor"
      ) as HTMLElement;
      if (container) {
