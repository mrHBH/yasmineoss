...
class LetterCounterAgent(BaseAIAgent):
    ...
    def __init__(self, llm: Any, llm_lock: asyncio.Lock, websocket: WebSocket, generator:  Optional[Any] = None):
        super().__init__(llm, llm_lock, websocket, generator)
        self.llm = models.LlamaCpp(llm)
        sampler = samplers.greedy()
        samplertext = samplers.multinomial(temperature=0.2)
        self.text_generator = generate.text(self.llm, sampler=sampler)
        self.text_generator_creative = generate.text(self.llm, sampler=samplertext )
    ...
    async def classify(self, question: str) -> bool:
        ...
        try:  
            ...
            USER: {question}\nAI:"""
            classification = self.text_generator(
                classification_question, max_tokens=2, stop_at="\n"
            )       
            # text =  "<|im_start|>system you are a creative story writer ai <|im_end|> <|im_start|>user generate a very short story about bob and alice. 5 sentences max.<|im_end|><|im_start|>assistant 'sure thing , here we go, so bob"  
            # classification =  self.text_generator_creative(text, max_tokens=250, stop_at="<|im_end|>",  ).strip()
    ...
    async def _stream_tokens(self, query: str) -> Optional[str]:
        ...
        jsonvar = ""
        tokens = self.generator.stream(query, max_tokens=1024)
    ...
    async def verify(self, question: str, word: str, letter: str) -> None:
        ...
        if len(letter_matches) == 0:         
            ...
            corrected_letter = self.text_generator(correction_query, max_tokens=2, stop_at="\n").strip()
            ...
            if len(letter_matches) == 0:
                ...
                "\n '"""
                final_corrected_letter = self.text_generator(correction_query, max_tokens=2, stop_at="\n").strip()
